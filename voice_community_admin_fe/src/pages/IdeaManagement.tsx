...
export default function IdeaManagement() {
  ...
  const loadVoices = async () => {
    ...
    try {
      ...
      // 筛选IDEA类型
      const ideaVoices = list.filter((v: Voice) => v.type === 'IDEA');
      setVoices(ideaVoices);
      setPagination(prev => ({ ...prev, total }));
    } catch (error: any) {
      console.error('加载创意列表失败:', error);
      message.error(error?.response?.data?.message || '加载创意列表失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="management-container">
      <Card className="management-header" styles={{ body: { padding: '24px' } }}>
        <div className="header-title">创意管理</div>
